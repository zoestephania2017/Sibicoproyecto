...
export default function openModal(e, dt, node, config, modal, buttonModal) {
    ...
    switch (action) {
        case '001':
            setModalTitle('modalTitle', 'Crear Año');
            setNameModalButton('btnModal', 'Guardar datos');
            buttonModal.click();
            break;
        case '002':
            setParameters(selectedRowData, modal, buttonModal, 'modalTitle', 'Editar Año', 'btnModal', 'Editar año');
            break;
        case '003':
            setFormReadOnly('frmYear');
            setParameters(selectedRowData, modal, buttonModal, 'modalTitle', 'Eliminar Año', 'btnModal', 'Eliminar año');
            break;
        default:
            console.log('error');
            break;
...
function setParameters(selectedRowData, modal, buttonModal, modalTitleID, modalTitle, buttonModalID, buttonName) {
    ...
    modal.querySelector('#txtYear').value = selectedRowData.nombre;
    modal.querySelector('#txtModel').value = selectedRowData.codigo_modelo;
...
export async function send(data = {}, option = '001') {
    const url = '../../app/controllers/YearController.php';
