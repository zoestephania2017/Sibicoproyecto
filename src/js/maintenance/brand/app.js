...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const formBrand = document.querySelector('#frmBrand');
    const actionModal = document.querySelector('#txtAction');
    const buttonClose = document.querySelector('#btnClose');

    let tableBrans = new DataTable('#tblBrands', {
        dom: 'Bfrtip',
        ajax: {
            url: '../../app/controllers/BrandController.php',
            type: 'GET',
            dataSrc: '',
        },
        pageLength: 16,
    ...
    formBrand.addEventListener('submit', async (e) => {
        ...
        switch (option) {
            case '001':
                if (validateForm()) {
                    let dataSave = {
                        nombre_marca: modal.querySelector('#txtName').value,
                    }
            ...
            case '002':
                ...
                break;
            case '003':
                swal("Estas seguro que deseas eliminar la marca?", {
