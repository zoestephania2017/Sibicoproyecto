...
class SaleReport extends FPDF
{
    ...
    public function addContentForID($id)
    {
        ...
        $sales = $this->model->getSale($id);
        $details = $sales[0]['details'];
        $winery = $this->model->getSession()->get('winery');

        $this->SetFont('Arial', 'B', 14);
        $this->Cell(0, 2, utf8_decode('ACTA DE ENTREGA'), 0, 0, 'C');
        $this->SetFont('Arial', 'B', 12);
        $this->Cell(0, 15, 'ACTA #', 0, 0, 'R');
        $this->Ln(9);
        $this->SetFont('Arial', '', 12);
        $this->Cell(0, 5, $sales[0]['numero'], 0, 0, 'R');
        ...
        $this->Cell(2, 5, formatDate($sales[0]['fecha_transaccion']), 0, 0, 'L');
        $this->Ln(1);
        ...
        $this->SetFont('Arial', '', 9);
        $this->Cell(2, 5, utf8_decode($sales[0]['cliente']), 0, 0, 'L');
        $this->Ln(3);

        $this->Ln(1);
        $this->Cell(1);
        $this->SetFont('Arial', 'B', 9);
        $this->Cell(25, 5, 'Codigo Inst: ', 0, 0, 'L');
        $this->SetFont('Arial', '', 9);
        $this->Cell(2, 5, utf8_decode($sales[0]['cod_cliente'] . " " . $sales[0]['cliente_completo']), 0, 0, 'L');
        $this->Ln(3);
        ...
        $this->SetFont('Arial', '', 9);
        $this->Cell(2, 5, utf8_decode($sales[0]['responsable']), 0, 0, 'L');
        $this->Ln(7);
        ...
        $this->MultiCell(0, 3, utf8_decode($sales[0]['nota']), 'C');
    ...
    {
        $headers = array('Serie', utf8_decode('Descripción'), 'Cantidad');
        ...
        foreach ($details as $detail) {
            ...
            $this->Cell($w[1], 4, utf8_decode($detail['nombre_producto']), 'LR');
