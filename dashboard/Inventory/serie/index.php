...
if (!isset($protection) || !$protection->has("14")) {
    header('Location: ../../?denied=true&_per=' . urlencode("14"), true, 302);
    exit;
}
?>
...
<head>
    ...
    <meta name="description" content="Administración de existencias, aquí podrás ver las existencias de los productos que se encuentran almacenados en la bodega.">
    <link rel="manifest" href="<?= url('manifest.json') ?>">
    <link rel="shortcut icon" href="<?= url('src/img/icon_192x192.webp') ?>" type="image/x-icon">
    <title>Almacenes Copeco - Series</title>
...
<body class="bg-gray-100">
            ...
            <div class="flex flex-col gap-4 w-full">
                ...
                <div class="w-full h-auto bg-slate-300 rounded p-4 shadow-lg">
                    <div class="rounded-lg bg-gray-100 p-3 overflow-x-auto">
                        <table id="tblSeries" class="cell-border compact stripe"></table>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div id="modal" tabindex="-1" data-modal-backdrop="static" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative w-full max-w-2xl max-h-full">
            <div class="relative bg-gray-300 rounded-lg shadow">
                ...
                <form action="#" id="frmSerie">
                    <div class="p-6 space-y-2">
                        <input type="hidden" id="txtAction">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="col-span-2">
                                <label for="txtCode" class="block mb-2 text-sm font-medium text-gray-900">Código del producto</label>
                                <input type="text" id="txtCode" class="block w-full px-4 py-2 text-gray-900 bg-gray-50 rounded-lg border border-gray-300" readonly>
                            </div>
                            <div class="col-span-2">
                                <label for="txtSerie" class="block mb-2 text-sm font-medium text-gray-900">Serie</label>
                                <div class="flex flex-row gap-2">
                                    <input type="text" id="txtSerie" class="flex-1 border border-gray-300 px-4 py-2 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full">
                                    <button type="button" id="btnGenerateSerie" class="text-white bg-emerald-700 hover:bg-emerald-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Generar</button>
                                </div>
                            </div>
                            <div class="col-span-1 md:col-span-2 hidden">
                                <label for="txtImage" class="block mb-2 text-sm font-medium text-gray-900">Imagen del producto</label>
                                <input type="file" id="txtImage" class="block w-full px-4 py-2 text-gray-900 bg-gray-50 rounded-lg border border-gray-300" disabled>
                            </div>
                        </div>
                    </div>
    ...
    <script src="<?= url('src/js/inventory/stocks/serie/app-min.js') ?>" type="module" defer></script>
</body>
