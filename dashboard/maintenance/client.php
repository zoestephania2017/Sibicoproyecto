...
if (!isset($protection) || !$protection->has("4")) {
    header('Location: ../?denied=true&_per=' . urlencode("4"), true, 302);
    exit;
}
?>
...
<head>
    ...
    <meta name="description" content="Administración de clientes, aquí podrás gestionar los datos de los clientes.">
    <link rel="manifest" href="<?= url('manifest.json') ?>">
    <link rel="shortcut icon" href="<?= url('src/img/icon_192x192.webp') ?>" type="image/x-icon">
    <!--  <title>Almacenes Copeco - Clientes</title> -->
    <title>Almacenes Copeco - Sucursales</title>
...
<body class="bg-gray-100">
    ...
    <div id="modal" tabindex="-1" data-modal-backdrop="static" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative w-full max-w-2xl max-h-full">
            <div class="relative bg-gray-300 rounded-lg shadow">
                ...
                <form action="#" id="frmClient">
                    <div class="p-6 space-y-2">
                        <input type="hidden" id="txtCode">
                        <input type="hidden" id="txtAction">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label for="txtName" class="block mb-2 text-sm font-medium text-gray-900">Nombres</label>
                                <input type="text" class="block w-full px-4 py-2 text-gray-900 bg-gray-50 rounded-lg border border-gray-300" id="txtName" />
                            </div>
                            <div>
                                <!-- <label for="txtLastName" class="block mb-2 text-sm font-medium text-gray-900">Apellidos</label> -->
                                <label for="txtLastName" class="block mb-2 text-sm font-medium text-gray-900">Ubicacion</label>
                                <input type="text" class="block w-full px-4 py-2 text-gray-900 bg-gray-50 rounded-lg border border-gray-300" id="txtLastName" />
                            </div>
                            <div class="md:col-span-2">
                                <label for="txtEmail" class="block mb-2 text-sm font-medium text-gray-900">Correo electrónico</label>
                                <input type="text" class="block w-full px-4 py-2 text-gray-900 bg-gray-50 rounded-lg border border-gray-300" id="txtEmail" />
                            </div>
                            <div class="md:col-span-2">
                                <label for="txtAddress" class="block mb-2 text-sm font-medium text-gray-900">Dirección</label>
                                <textarea id="txtAddress" cols="30" rows="5" class="block w-full px-4 py-2 text-gray-900 bg-gray-50 rounded-lg border border-gray-300"></textarea>
                            </div>
                            <div>
                    ...
                    <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b">
                        ...
                        <button data-modal-hide="modal" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 shadow-lg" type="button" id="btnCancelEdit">Cancelar</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script src="<?= url('src/js/maintenance/client/app-min.js') ?>" type="module" defer></script>
</body>
